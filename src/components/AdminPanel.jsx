...
const AdminPanel = () => {
  ...
  const [editingSubId, setEditingSubId] = useState(null); // null = new, id = edit
  ...
  const handleSubSubmit = async (e) => {
    e.preventDefault();
    if (!subForm.phone_number || !subForm.customer_name || !subForm.plan_name) {
    ...
    try {
      if (editingSubId) {
        await updateSubscription(editingSubId, subForm);
        triggerAlert("Subscription updated successfully!");
      } else {
        await addSubscription(subForm);
        triggerAlert("Subscription registered successfully!");
      }
      ...
      triggerAlert("Failed to save subscription.", "error");
    }
  };

  const handleMealChange = async (sub, delta) => {
    const nextVal = sub.meals_remaining + delta;
    if (nextVal < 0 || nextVal > sub.meals_total) return;

    try {
      await updateSubscription(sub.id, { meals_remaining: nextVal });
    } catch (err) {
      triggerAlert("Failed to update meals count.", "error");
    }
  };
    ...
    <div style={{
      ...
      <div className="container" style={{
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '8px' }} className="admin-sidebar">
          ...
          <button 
            onClick={() => { setActiveTab('subscriptions'); setIsPlanFormOpen(false); setIsSaladFormOpen(false); setIsSubFormOpen(false); }}
            style={{
              ...sidebarBtnStyle,
            ...
            <div>
              {!isSubFormOpen ? (
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    <div>
                      <h3 style={{ fontSize: '22px', fontWeight: 700, color: 'var(--text-main)', marginBottom: '6px' }}>Active Subscribers</h3>
                      <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Create, monitor, and update salad subscription meal counts for active customers.</p>
                    </div>
                    <button 
                        ...
                        {filteredSubs.map((sub) => {
                          ...
                          return (
                            <tr key={sub.id} style={{ borderBottom: '1px solid var(--border-color)', transition: 'background-color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = 'var(--primary-light)'} onMouseOut={(e) => e.currentTarget.style.backgroundColor = 'transparent'}>
                              ...
                              <td style={{ padding: '16px', width: '240px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                  {/* Quick Decrease */}
                                  <button 
                                    onClick={() => handleMealChange(sub, -1)}
                                    disabled={sub.meals_remaining <= 0}
                                    style={{
                                      border: '1px solid var(--border-color)',
                                  ...
                                  <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', fontWeight: 700, marginBottom: '4px' }}>
                                      <span style={{ color: 'var(--text-main)' }}>{sub.meals_remaining} Remaining</span>
                                      <span style={{ color: 'var(--text-muted)' }}>/ {sub.meals_total} Total</span>
                                    </div>
                                  ...
                                  <button 
                                    onClick={() => handleMealChange(sub, 1)}
                                    disabled={sub.meals_remaining >= sub.meals_total}
                                    style={{
                                      border: '1px solid var(--border-color)',
                                      background: 'var(--card-bg)',
                                      borderRadius: '4px',
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    try {
                                      ...
                                      triggerAlert(`Status updated to ${nextStatus}.`);
                                    } catch (err) {
                                      triggerAlert("Failed to update status.", "error");
                                    }
                                  }}
                              ...
                              <td style={{ padding: '16px', textAlign: 'center' }}>
                                <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                                  ...
                                  <button 
                                    onClick={async () => {
                                      if (window.confirm(`Are you sure you want to delete the subscription for ${sub.customer_name}?`)) {
                                        try {
                                          await deleteSubscription(sub.id);
                                          triggerAlert("Subscription deleted successfully.");
                                        } catch (err) {
                                          triggerAlert("Failed to delete subscription.", "error");
                                        }
                                      }
                                    }}
                                    style={{ ...actionBtnStyle, color: 'var(--danger)', borderColor: 'rgba(239, 68, 68, 0.2)' }}
                ...
                <form onSubmit={handleSubSubmit} style={{ maxWidth: '540px' }}>
                  <h3 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--primary-dark)', marginBottom: '8px' }}>
                    {editingSubId ? 'Modify Subscription Record' : 'Register New Subscriber'}
                  </h3>
                  <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '24px' }}>
                    Fill out customer contact information and configured subscription details below.
                  </p>
                  ...
                  <div className="admin-input-group">
                    ...
                    <input 
                      type="tel" 
                      value={subForm.phone_number}
                      onChange={(e) => setSubForm({...subForm, phone_number: e.target.value})}
                  ...
                  <div className="admin-input-group">
                    <label className="admin-label">Salad Plan / Custom Package *</label>
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <select 
                        ...
                        required
                      >
                        <option value="">-- Select preconfigured plan --</option>
                        {saladPlans.map(p => (
                          <option key={p.id} value={p.title}>{p.title} ({p.meals_count} meals)</option>
                        ))}
                        <option value="Custom Combo Plan">Custom Combo Plan</option>
                      </select>
                      <input 
                        type="text"
                        placeholder="Or type custom package name..."
                        value={subForm.plan_name}
                        onChange={(e) => setSubForm({...subForm, plan_name: e.target.value})}
                        className="admin-input"
                        style={{ flex: 1 }}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <div className="admin-input-group">
                      <label className="admin-label">Total Meals (Plan Length)</label>
                      <input 
                        type="number" 
                        min="1"
                        value={subForm.meals_total}
                        onChange={(e) => {
                          const total = parseInt(e.target.value, 10) || 10;
                          setSubForm({...subForm, meals_total: total, meals_remaining: Math.min(subForm.meals_remaining, total)});
                        }}
                        className="admin-input"
                      />
                    </div>
                    <div className="admin-input-group">
                      <label className="admin-label">Remaining Meals Count</label>
                      <input 
                        type="number" 
                        min="0"
                        max={subForm.meals_total}
                        value={subForm.meals_remaining}
                        onChange={(e) => setSubForm({...subForm, meals_remaining: Math.min(subForm.meals_total, parseInt(e.target.value, 10) || 0)})}
                        className="admin-input"
                      />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '16px', marginTop: '30px' }}>
                    ...
                    >
                      <Save size={16} />
                      {editingSubId ? 'Update Subscription' : 'Register Subscriber'}
                    </button>
                  </div>
                </form>
              )}
